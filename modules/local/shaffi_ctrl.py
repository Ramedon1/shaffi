#!/usr/bin/env python3
# shaffi_ctrl.py — управление BPF-картами eBPF-шейпера через pinned path.
# Поддерживает до 32 независимых правил (Multi-Rule).
...
DEFAULT_PIN_DIR    = "/sys/fs/bpf/shaffi/maps"
DEFAULT_RULES_FILE = "/etc/shaffi/traffic_limiter/rules.json"
...
def bpftool_map_update(pin_dir, map_name, key_hex, value_hex):
    ...
    if not os.path.exists(pin_path):
        ...
        print(f"   Убедись что сервис запущен: systemctl status shaffi-traffic-limiter")
        sys.exit(1)
...
# struct rule_config layout (little-endian):
#   __u32 mode;                       offset 0
#   __u32 num_ports;                  offset 4
#   __u32 ports[MAX_PORTS=32];        offset 8  (128 bytes)
#   __u64 down_rate_bps;              offset 136
#   __u64 up_rate_bps;                offset 144
#   __u64 penalty_rate_bps;           offset 152
#   __u64 burst_bytes_limit;          offset 160
#   __u64 window_time_ns;             offset 168
#   __u64 penalty_time_ns;            offset 176
#   Total = 184 bytes
...
def get_mode_str(mode):
    return {1: "Статика", 2: "Динамика", 3: "Общее ограничение"}.get(mode, "Выкл")

def get_mode_str_color(mode):
    return {
        1: "\033[0;32mСтатика\033[0m",
        2: "\033[0;33mДинамика\033[0m",
        3: "\033[0;36mОбщее ограничение\033[0m",
    }.get(mode, "\033[0;31mВыкл\033[0m")
        ...
        with open(whitelist_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if key_hex:
                    ...
                    print(f"⚠️  Неверный IP в белом списке: {ip_part}")

    print(f"✅ Белый список: загружено {count} IP-адресов.")
...
def list_rules(pin_dir, rules_file=DEFAULT_RULES_FILE):
    ...
    for rid_str, r in sorted(rules.items(), key=lambda x: int(x[0])):
        ...
        print(f"  ✔ Правило \033[1;37m#{rid_str}\033[0m | {mode_str}")
        print(f"    Порты : {ports_str}")
        print(f"    DL/UL : {r['down_mbs']} / {r['up_mbs']} МБ/с")
        if r['mode'] == 2:
            print(f"    Burst : {r['burst_mb']} МБ в {r['win_sec']}с | "
                  f"Штраф: {r.get('pen_mbs',0.1)} МБ/с на {r['pen_sec']}с")
        print(f"  {sep}")
...
def get_ip_from_rule_key(key):
    if isinstance(key, dict):
        ...
        parts   = [r32(key, i*4) for i in range(4)]
        rule_id = r32(key, 16)
    else:
        return "?.?.?.?", 0
...
def dump_stats(pin_dir, rule_filter=None, full=False, rules_file=DEFAULT_RULES_FILE):
    ...
    for rule_id_str in sorted(rules_saved.keys(), key=lambda x: int(x)):
        ...
        print(f"  {sep}")
        hdr = f"  {'IP-адрес':<30} {'↓ Скачано':<13} {'↑ Загружено':<13} {'Лимит'}"
        ...
        for idx, ip in enumerate(display):
            ...
            if mode == 2 and is_pen:
                limit = f"\033[0;31m⚡ ШТРАФ {pen_mbs:.1f} МБ/с\033[0m"
            elif d_mbs:
                limit = f"\033[0;32m✔ {d_mbs:.1f} МБ/с\033[0m"
            else:
                limit = "⚠ неизв."
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="shaffi_ctrl — управление eBPF Traffic Shaper (Multi-Rule)")
    ...
    p_set.add_argument("--rule-id", type=int, required=True)
    p_set.add_argument("--mode",    type=int, choices=[1, 2, 3], required=True)
    p_set.add_argument("--ports",   type=str, default="0")
    p_set.add_argument("--down",    type=float, required=True)
    p_set.add_argument("--up",      type=float, required=True)
    p_set.add_argument("--pen",     type=float, default=0.1)
    p_set.add_argument("--burst",   type=float, default=70.0)
    p_set.add_argument("--win",     type=int,   default=10)
    p_set.add_argument("--pen-sec", type=int,   default=60)
    ...
    p_wl = sub.add_parser("whitelist-sync", help="Синхронизировать белый список")
    p_wl.add_argument("--file", type=str, nargs='+', required=True)

    sub.add_parser("restore", help="Восстановить правила из rules.json (для ExecStart сервиса)")

    p_stat = sub.add_parser("status", help="Статистика")
